                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <div class="col-span-2">                                           
                                        ...
                                        @if ($user->profile_image)
                                            <img src="{{ asset('storage/profile_images/' . $user->profile_image) }}" alt="Profile Image" class="mb-2 w-32 h-32 object-cover rounded-full">
                                        @endif
                                        <input name="profile_image" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" id="edit-file_input" type="file">
                                    </div>
                                    <div class="col-span-2">
...
document.addEventListener('DOMContentLoaded', function () {
        ...
        searchInput.addEventListener('input', function () {
            ...
            tableRows.forEach(row => {
                const nameColumn = row.children[1].textContent.toLowerCase();
                const usernameColumn = row.children[2].textContent.toLowerCase();
                const emailColumn = row.children[3].textContent.toLowerCase();
                const roleColumn = row.children[4].textContent.toLowerCase();

                const isMatch =
                    nameColumn.includes(searchValue) ||
                    usernameColumn.includes(searchValue) ||
                    emailColumn.includes(searchValue) ||
                    roleColumn.includes(searchValue);
