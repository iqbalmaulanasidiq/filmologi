...
<div class="dark:bg-gray-900">
    ...
    <div id="animation-carousel" class="relative w-full" data-carousel="slide">
        ...
        <div class="relative h-96 overflow-hidden rounded-lg md:h-screen">
            ...
            @foreach ($bannerArrayTV as $bannerItem)
                @php
                    $tvTrendingId = $bannerItem['id'];
                ...
                <div class="hidden duration-200 ease-linear" data-carousel-item>
                    ...
                    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white text-center">
                        <h1 class="text-5xl font-extrabold tracking-tight leading-none font-serif mb-4">{{ $bannerItem['name'] }}</h1>
                        ...
                        <button class="mx-auto px-8 py-3 bg-blue-500 text-white rounded-full hover:bg-blue-600 hover:text-gray-100 flex items-center">
                            ...
                            <a href="tv-shows/{{ $tvTrendingId }}" class="font-semibold text-xl">Lihat Detail</a>
                        </button>
                    </div>
                </div>
            @endforeach
        ...
        <div class="flex justify-between items-center mb-4">
         ...
         {{-- start card --}}
            <div class="flex flex-wrap items-stretch overflow-hidden -mx-4 mt-4 pt-10" id="dataWrapper">
                @foreach ($TVArray as $TVItems)
                    @php
                        $original_date = $TVItems['first_air_date'];
                        $timestamp = strtotime($original_date);

                        $TVYear = date("Y", $timestamp);
                        $TVId = $TVItems['id'];
                        $TVTitle = $TVItems['name'];
                        $TVRating = $TVItems['vote_average'] * 10;
                        $TVImage = "{$imageBaseUrl}/w500{$TVItems['poster_path']}";
                    @endphp
